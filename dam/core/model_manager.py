...
from typing import Any, Dict, Optional, Callable, Tuple, FrozenSet, TypeVar, Generic
...
class ModelExecutionManager(Generic[ModelType]):
    ...
    def __init__(self):
        ...
        logger.info(f"ModelExecutionManager initialized.")
    ...
    async def get_model(
        ...
        try:
            ...
            logger.error(f"Failed to load model '{model_name_or_path}' (identifier: {model_identifier}) with params {params}: {e}", exc_info=True)
            raise
    ...
    def get_optimal_batch_size(
        ...
        if model_identifier in self._batch_sizers:
            ...
            try:
                ...
                logger.error(f"Error in specific batch sizer for {model_identifier} ('{model_name_or_path}'): {e}. Falling back to default.", exc_info=True)
        ...
        try:
            if torch.cuda.is_available():
                ...
                logger.info(f"Default sizer: Apple MPS detected for '{model_name_or_path}'. Using {available_vram_gb:.2f}GB (25% of system RAM) as proxy.")
                 ...
                 logger.info(f"Default sizer: Using estimated VRAM for {self.gpu_info['devices'][0]['name']} for '{model_name_or_path}': {available_vram_gb:.2f}GB.")
            else:
                logger.info(f"Default sizer: Could not determine VRAM for '{model_name_or_path}'. Using fallback {fallback_batch_size}.")
                return fallback_batch_size
        except Exception as e:
            logger.warning(f"Default sizer: Could not query VRAM for '{model_name_or_path}': {e}. Using total VRAM as approximation.", exc_info=True)
        ...
        logger.info(f"Default sizer: Estimated optimal batch for '{model_name_or_path}' (item ~{item_size_estimate_mb:.2f}MB): {optimal_batch} "
