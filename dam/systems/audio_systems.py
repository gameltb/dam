...
from dam.core.system_params import WorldSession, WorldContext
from dam.core.systems import system
from dam.services import ecs_service, audio_service
...
from dam.models.conceptual.base_conceptual_info_component import BaseConceptualInfoComponent # Using a base for example
...
async def audio_embedding_generation_system(
    ...
    for entity in entities_to_process:
        ...
        if not file_props or not file_props.mime_type or not file_props.mime_type.startswith("audio/"):
            logger.warning(f"Entity {entity.id} marked for audio processing, but not found or not an audio MIME type ({file_props.mime_type if file_props else 'N/A'}). Skipping.")
            continue
        ...
        existing_embeddings = await ecs_service.get_components_by_value(
            ...
            entity.id,
            AudioEmbeddingClass,
            attributes_values={"model_name": default_model_name}
        )
