...
from typing import Annotated, Any, List, Dict # Added Dict
...
async def extract_metadata_on_asset_ingested(
    ...
    for entity in entities_to_process:
        ...
        mime_type = file_props.mime_type if file_props else "application/octet-stream" # Default if fpc is None (though checked above)
        ...
        if not metadata:
            ...
            logger.debug(f"Hachoir metadata (type: {type(metadata).__name__}) keys for {filepath_on_disk}: {keys_to_log}")
            ...
            is_video_heuristic = mime_type.startswith("video/") or \
                                 (mime_type.startswith("image/") and _get_hachoir_metadata(metadata, "nb_frames", 0) > 1 and has_duration) or \
                                 (has_duration and (has_width or has_frame_rate) and not mime_type.startswith("audio/"))
            ...
            if is_audio_file_heuristic: # Standalone audio file
                if not ecs_service.get_components(session, entity.id, AudioPropertiesComponent):
                    ...
                    if not audio_codec_val and _has_hachoir_metadata(metadata, "compression"): # Fallback for some formats
            ...
            if is_video_heuristic: # Video file or animated image with frames/duration
                if not ecs_service.get_components(session, entity.id, FramePropertiesComponent):
                    ...
                    if not video_frame_comp.frame_count and \
                       video_frame_comp.nominal_frame_rate and \
                       video_frame_comp.animation_duration_seconds and video_frame_comp.animation_duration_seconds > 0:
            ...
            if mime_type == "image/gif":
                if not ecs_service.get_components(session, entity.id, FramePropertiesComponent): # Check again if not caught by video heuristic
                    frame_comp = FramePropertiesComponent(entity_id=entity.id, entity=entity)
                    nb_frames_gif = _get_hachoir_metadata(metadata, "nb_frames") or _get_hachoir_metadata(metadata, "frame_count")
