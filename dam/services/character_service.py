...
async def find_character_concepts(
    session: AsyncSession, query_name: Optional[str] = None
) -> List[Entity]:
...
async def get_characters_for_entity(
    ...
    stmt = (
        select(EntityCharacterLinkComponent.character_concept_entity_id, EntityCharacterLinkComponent.role_in_asset)
