...
class FileLocationComponent(BaseComponent): # BaseComponent inherits from Base(MappedAsDataclass)
    ...
    # uri: Mapped[Optional[str]] = mapped_column(String(2048), nullable=True) # Optional full URI if different from filepath
    ...
    def __repr__(self):
        return f"FileLocationComponent(id={self.id}, entity_id={self.entity_id}, type='{self.storage_type}', path='{self.filepath}')"
