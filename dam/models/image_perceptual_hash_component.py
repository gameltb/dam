...
class ImagePerceptualHashComponent(BaseComponent): # BaseComponent already inherits from Base
    ...
    hash_value: Mapped[str] = mapped_column(String(256), index=True, nullable=False) # Perceptual hashes can be hex strings
    ...
    __table_args__ = (
        UniqueConstraint("entity_id", "hash_type", name="uq_image_perceptual_hash_entity_type"),
    )

    def __repr__(self):
        return f"ImagePerceptualHashComponent(id={self.id}, entity_id={self.entity_id}, hash_type='{self.hash_type}', hash_value='{self.hash_value[:10]}...')"
