...
class ContentHashComponent(BaseComponent): # BaseComponent already inherits from Base
    ...
    __table_args__ = (
        UniqueConstraint("entity_id", "hash_type", name="uq_content_hash_entity_type"),
    )

    def __repr__(self):
        return f"ContentHashComponent(id={self.id}, entity_id={self.entity_id}, hash_type='{self.hash_type}', hash_value='{self.hash_value[:10]}...')"
