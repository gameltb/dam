...
class WebsiteProfileComponent(BaseComponent):
    ...
    description: Mapped[Optional[str]] = mapped_column(Text, comment="Optional description of the website.")
    icon_url: Mapped[Optional[str]] = mapped_column(String(2048), comment="URL to a favicon or logo for the website.")

    # For potential API interactions or specific scraping configurations
    api_endpoint: Mapped[Optional[str]] = mapped_column(
        String(2048), comment="Primary API endpoint for the website, if applicable."
    )
