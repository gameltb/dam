...
@pytest_asyncio.fixture
async def global_tag_concept(db_session: AsyncSession) -> Entity:
    """Provide a global tag concept for testing."""
    tag_entity = await ts.create_tag_concept(
...
@pytest_asyncio.fixture
async def component_scoped_tag_concept(db_session: AsyncSession) -> Entity:
    """Provide a component-scoped tag concept for testing."""
...
@pytest_asyncio.fixture
async def local_scoped_tag_concept_for_comic1(db_session: AsyncSession, comic_concept1: Entity) -> Entity:
    """Provide a local-scoped tag concept for testing."""
...
@pytest_asyncio.fixture
async def comic_concept1(db_session: AsyncSession) -> Entity:
    """Provide a comic book concept for testing."""
    concept = await cbs.create_comic_book_concept(db_session, comic_title="Test Comic Alpha", issue_number="1")
    assert concept is not None
    return concept


@pytest_asyncio.fixture
async def comic_variant1_of_concept1(db_session: AsyncSession, comic_concept1: Entity) -> Entity:
    """Provide a comic book variant for testing."""
    # comic_concept1 fixture is now async
    variant_file_entity = await ecs_service.create_entity(db_session)
...
@pytest_asyncio.fixture
async def generic_entity1(db_session: AsyncSession) -> Entity:
...
@pytest.mark.asyncio
async def test_create_tag_concept(db_session: AsyncSession) -> None:
    """Test creating a tag concept."""
    ...
    comp = await ecs_service.get_component(db_session, tag_entity.id, TagConceptComponent)
    ...
    # Test duplicate name
    assert await ts.create_tag_concept(db_session, tag_name, "GLOBAL") is tag_entity  # Should return existing

    # Test empty name
    with pytest.raises(ValueError, match="Tag name cannot be empty"):
        await ts.create_tag_concept(db_session, "", "GLOBAL")


@pytest.mark.asyncio
async def test_get_tag_concept_by_name(db_session: AsyncSession, global_tag_concept: Entity) -> None:
    """Test getting a tag concept by name."""
    retrieved_tag = await ts.get_tag_concept_by_name(db_session, "GlobalTestTag")
    assert retrieved_tag is not None
    assert retrieved_tag.id == global_tag_concept.id
    with pytest.raises(ts.TagConceptNotFoundError):  # Expect an exception
        await ts.get_tag_concept_by_name(db_session, "NonExistentTag")


@pytest.mark.asyncio
async def test_get_tag_concept_by_id(db_session: AsyncSession, global_tag_concept: Entity) -> None:
    """Test getting a tag concept by ID."""
    retrieved_tag = await ts.get_tag_concept_by_id(db_session, global_tag_concept.id)
    assert retrieved_tag is not None
    assert retrieved_tag.id == global_tag_concept.id
    assert await ts.get_tag_concept_by_id(db_session, 99999) is None
...
async def test_find_tag_concepts(
    ...
    """Test finding tag concepts."""
    all_tags = await ts.find_tag_concepts(db_session)
    assert len(all_tags) >= 2  # At least the two created by fixtures

    global_tags = await ts.find_tag_concepts(db_session, scope_type="GLOBAL")
    assert global_tag_concept in global_tags
    assert component_scoped_tag_concept not in global_tags

    named_tags = await ts.find_tag_concepts(db_session, query_name="GlobalTest")
    assert len(named_tags) == 1
    assert named_tags[0].id == global_tag_concept.id


@pytest.mark.asyncio
async def test_update_tag_concept(db_session: AsyncSession, global_tag_concept: Entity) -> None:
    """Test updating a tag concept."""
    updated_comp = await ts.update_tag_concept(
    ...
    await ts.create_tag_concept(db_session, "AnotherTag", "GLOBAL")
    assert await ts.update_tag_concept(db_session, global_tag_concept.id, name="AnotherTag") is None  # Should fail

    # Test clear description
    await ts.update_tag_concept(db_session, global_tag_concept.id, description="__CLEAR__")
    await db_session.refresh(updated_comp)
    assert updated_comp.tag_description is None


@pytest.mark.asyncio
async def test_delete_tag_concept(db_session: AsyncSession, generic_entity1: Entity) -> None:
    """Test deleting a tag concept."""
    tag_to_delete_entity = await ts.create_tag_concept(db_session, "ToDeleteTag", "GLOBAL")
    assert tag_to_delete_entity is not None
    tag_id = tag_to_delete_entity.id

    # Apply it first
    with db_session.no_autoflush:
        await ts.apply_tag_to_entity(db_session, generic_entity1.id, tag_id)
    assert await ecs_service.get_component(db_session, generic_entity1.id, EntityTagLinkComponent) is not None

    assert await ts.delete_tag_concept(db_session, tag_id) is True
    assert await ts.get_tag_concept_by_id(db_session, tag_id) is None
    # Check if link component was cascade deleted
    assert await ecs_service.get_component(db_session, generic_entity1.id, EntityTagLinkComponent) is None
...
async def test_apply_and_get_tags_for_entity(
    ...
    """Test applying and getting tags for an entity."""
    # Apply global tag
    with db_session.no_autoflush:
        link1 = await ts.apply_tag_to_entity(db_session, generic_entity1.id, global_tag_concept.id)
    ...
    value_tag_concept = await ts.create_tag_concept(db_session, "ValueTag", "GLOBAL", allow_values=True)
    assert value_tag_concept is not None
    with db_session.no_autoflush:
        link2 = await ts.apply_tag_to_entity(db_session, generic_entity1.id, value_tag_concept.id, value="TestValue")
    assert link2 is not None
    assert link2.tag_value == "TestValue"

    # Get tags for entity
    applied_tags = await ts.get_tags_for_entity(db_session, generic_entity1.id)
...
async def test_remove_tag_from_entity(
    ...
    with db_session.no_autoflush:
        await ts.apply_tag_to_entity(db_session, generic_entity1.id, global_tag_concept.id)
    assert len(await ts.get_tags_for_entity(db_session, generic_entity1.id)) == 1

    assert await ts.remove_tag_from_entity(db_session, generic_entity1.id, global_tag_concept.id) is True
    assert len(await ts.get_tags_for_entity(db_session, generic_entity1.id)) == 0
    assert (
...
@pytest.mark.asyncio
async def test_get_entities_for_tag(
    db_session: AsyncSession, global_tag_concept: Entity, generic_entity1: Entity, comic_concept1: Entity
) -> None:
    """Test getting entities for a tag."""
    with db_session.no_autoflush:
        await ts.apply_tag_to_entity(db_session, generic_entity1.id, global_tag_concept.id)
        await ts.apply_tag_to_entity(db_session, comic_concept1.id, global_tag_concept.id)

    value_tag = await ts.create_tag_concept(db_session, "Status", "GLOBAL", allow_values=True)
    assert value_tag is not None
    with db_session.no_autoflush:
        await ts.apply_tag_to_entity(db_session, generic_entity1.id, value_tag.id, value="Draft")
        await ts.apply_tag_to_entity(db_session, comic_concept1.id, value_tag.id, value="Published")

    entities_with_global_tag = await ts.get_entities_for_tag(db_session, global_tag_concept.id)
    ...
    entities_with_status_draft = await ts.get_entities_for_tag(db_session, value_tag.id, value_filter="Draft")
    ...
    with db_session.no_autoflush:
        await ts.apply_tag_to_entity(db_session, entity3.id, value_tag.id, value=None)  # Valueless application
...
@pytest.mark.asyncio
async def test_scope_validation_component_class_required(
    db_session: AsyncSession,
    component_scoped_tag_concept: Entity,  # Scope: ComicBookConceptComponent
    comic_concept1: Entity,
    generic_entity1: Entity,
) -> None:
    """Test scope validation for component class required."""
    # Should succeed: comic_concept1 has ComicBookConceptComponent
    with db_session.no_autoflush:
        link_success = await ts.apply_tag_to_entity(db_session, comic_concept1.id, component_scoped_tag_concept.id)
    assert link_success is not None

    # Should fail: generic_entity1 does not have ComicBookConceptComponent
    with db_session.no_autoflush:
        link_fail = await ts.apply_tag_to_entity(db_session, generic_entity1.id, component_scoped_tag_concept.id)
    assert link_fail is None


@pytest.mark.asyncio
async def test_scope_validation_conceptual_asset_local(
    db_session: AsyncSession,
    comic_concept1: Entity,
    comic_variant1_of_concept1: Entity,
    local_scoped_tag_concept_for_comic1: Entity,  # Scope: local to comic_concept1
) -> None:
    """Test scope validation for conceptual asset local."""
    ...
    with db_session.no_autoflush:
        link_on_variant = await ts.apply_tag_to_entity(
    ...
    unrelated_entity = await ecs_service.create_entity(db_session)
    with db_session.no_autoflush:
    ...
    comic_concept2 = await cbs.create_comic_book_concept(db_session, comic_title="Unrelated Comic")
    with db_session.no_autoflush:
        link_on_other_concept = await ts.apply_tag_to_entity(
...
@pytest.mark.asyncio
async def test_scope_validation_invalid_scope_details(db_session: AsyncSession, generic_entity1: Entity) -> None:
    """Test scope validation with invalid scope details."""
    # COMPONENT_CLASS_REQUIRED with no detail
    no_detail_comp_tag = await ts.create_tag_concept(
        db_session, "NoDetailCompTag", "COMPONENT_CLASS_REQUIRED", scope_detail=None
    )
    assert no_detail_comp_tag is not None
    assert await ts.apply_tag_to_entity(db_session, generic_entity1.id, no_detail_comp_tag.id) is None

    # COMPONENT_CLASS_REQUIRED with non-existent component class name
    bad_detail_comp_tag = await ts.create_tag_concept(
    ...
    assert await ts.apply_tag_to_entity(db_session, generic_entity1.id, bad_detail_comp_tag.id) is None

    # CONCEPTUAL_ASSET_LOCAL with no detail
    no_detail_local_tag = await ts.create_tag_concept(
        db_session, "NoDetailLocalTag", "CONCEPTUAL_ASSET_LOCAL", scope_detail=None
    )
    assert no_detail_local_tag is not None
    assert await ts.apply_tag_to_entity(db_session, generic_entity1.id, no_detail_local_tag.id) is None

    # CONCEPTUAL_ASSET_LOCAL with non-integer detail
    bad_detail_local_tag = await ts.create_tag_concept(
        db_session, "BadDetailLocalTag", "CONCEPTUAL_ASSET_LOCAL", scope_detail="not-an-int"
    )
    assert bad_detail_local_tag is not None
    assert await ts.apply_tag_to_entity(db_session, generic_entity1.id, bad_detail_local_tag.id) is None

    # CONCEPTUAL_ASSET_LOCAL where scope_detail ID is not a conceptual asset
    non_concept_owner = await ecs_service.create_entity(
        db_session
    )  # This entity does not have a BaseConceptualInfoComponent subclass
    local_to_non_concept_tag = await ts.create_tag_concept(
    ...
    assert await ts.apply_tag_to_entity(db_session, generic_entity1.id, local_to_non_concept_tag.id) is None


@pytest.mark.asyncio
async def test_unknown_scope_type(db_session: AsyncSession, generic_entity1: Entity) -> None:
    """Test that an unknown scope type is handled correctly."""
    unknown_scope_tag = await ts.create_tag_concept(db_session, "UnknownScopeTag", "MY_CUSTOM_SCOPE_TYPE")
    assert unknown_scope_tag is not None
    # Default behavior for unknown scope is to deny application
    assert await ts.apply_tag_to_entity(db_session, generic_entity1.id, unknown_scope_tag.id) is None
