...
class WebsiteProfileComponent(BaseComponent):
    ...
    main_url: Mapped[str] = mapped_column(String(2048), unique=True, index=True, nullable=False, comment="Main homepage URL of the website.")
    ...
    api_endpoint: Mapped[Optional[str]] = mapped_column(String(2048), comment="Primary API endpoint for the website, if applicable.", default=None)
    ...
    def __repr__(self):
        return f"WebsiteProfileComponent(id={self.id}, entity_id={self.entity_id}, " f"name='{self.name}', main_url='{self.main_url}')"
