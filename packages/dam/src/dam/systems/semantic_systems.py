...
from dam.services import audio_service, semantic_service
...
async def generate_embeddings_system(
    ...
    #        session, entity_id, {}, model_execution_manager, # Pass MEM
    #        model_name=model_name_to_use, batch_texts=texts_to_batch_for_entity
    #    )
...
async def handle_audio_search_query(
    ...
    if not event.result_future:
        logger.error(f"Result future not set on AudioSearchQuery event (Req ID: {event.request_id}). Cannot proceed.")
        return
