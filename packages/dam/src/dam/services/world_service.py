...
def get_all_component_classes() -> list[Type[BaseComponent]]:
    ...
    if not REGISTERED_COMPONENT_TYPES:
        logger.warning(
            "REGISTERED_COMPONENT_TYPES is empty. Ensure component models are imported and BaseComponent's registration mechanism is active."
        )
        ...
        for comp_type_name, comp_list_data in components_data.items():
            ...
            for comp_data in comp_list_data:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to create/add component {comp_type_name} for entity {entity_to_update.id} with data {comp_data_cleaned}: {e}",
                        exc_info=True,
                    )
...
def merge_ecs_worlds_db_to_db(
    ...
    try:
            ...
            for ComponentClass in all_component_types:
                ...
                for src_comp_instance in src_components:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Copied component {ComponentClass.__name__} from src_entity {src_entity.id} to tgt_entity {tgt_entity.id}."
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to copy component {ComponentClass.__name__} for src_entity {src_entity.id} to tgt_entity {tgt_entity.id}: {e}",
                            exc_info=True,
                        )
        ...
        logger.info(
            f"Successfully merged {source_entity_count} entities from world '{source_world.name}' to '{target_world.name}' using 'add_new' strategy."
        )
    except Exception as e:
...
def split_ecs_world(
    ...
    """
    logger.info(
        f"Starting DB-to-DB split from world '{source_world.name}' to selected='{target_world_selected.name}', remaining='{target_world_remaining.name}'."
    )
