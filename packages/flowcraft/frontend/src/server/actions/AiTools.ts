...
import { ActionTemplateSchema, ActionExecutionStrategy } from "../../generated/flowcraft/v1/core/action_pb";
...
import { TaskUpdateSchema, NodeSchema } from "../../generated/flowcraft/v1/core/node_pb";
import { MutationListSchema, StreamChunkSchema } from "../../generated/flowcraft/v1/core/service_pb";
...
async function runAiGeneration(req: ActionHandlerContext, instruction: string) {
  ...
  const maxY = Math.max(...targetNodes.map((n) => n.position.y + (n.measured?.height ?? 200)), 0);
  const avgX = targetNodes.reduce((acc, n) => acc + n.position.x, 0) / (targetNodes.length || 1);

  eventBus.emit("taskUpdate", create(TaskUpdateSchema, {
    taskId,
    status: 1,
    progress: 10,
  ...
  const newNode = fromProtoNode(create(NodeSchema, {
    nodeId: newNodeId,
    nodeKind: 1,
  ...
  eventBus.emit("mutations", create(MutationListSchema, {
    mutations: [{ operation: { case: "addNode", value: { node: toProtoNode(newNode) } } }],
  ...
  try {
    ...
    for await (const chunk of stream) {
      ...
      if (content) {
        eventBus.emit("streamChunk", create(StreamChunkSchema, {
          nodeId: newNodeId,
          widgetId: "logs",
          chunkData: content,
          isDone: false,
    ...
    eventBus.emit("taskUpdate", create(TaskUpdateSchema, { taskId, status: 2, progress: 100, message: "AI complete" }));
  } catch (err: any) {
    eventBus.emit("taskUpdate", create(TaskUpdateSchema, { taskId, status: 3, message: `Error: ${err instanceof Error ? err.message : String(err)}` }));
  }
}
...
NodeRegistry.registerGlobalAction(
  ...
  (ctx) => runAiGeneration(ctx, ctx.params.instruction ?? "Context aware transform")
);