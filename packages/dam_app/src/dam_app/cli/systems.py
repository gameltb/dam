...
async def process_archives(
    ...
    for entity_id in archives_to_process:
        ...
        while True:
            try:
                cmd = ProcessArchiveCommand(entity_id=entity_id, passwords=known_passwords)
                ...
                typer.secho(f"  An unexpected error occurred while processing archive {entity_id}: {e}", fg=typer.colors.RED)
                break
