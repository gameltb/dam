...
async def test_extract_metadata_from_file(mock_transaction: MagicMock, mocker: MockerFixture):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with mocker.patch.object(exiftool_instance, "get_metadata", new_callable=mocker.AsyncMock) as mock_get_metadata:
            mock_get_metadata.return_value = {"FileType": "TXT"}

            await extract_metadata_command_handler(command, mock_transaction, mock_world)

            mock_get_metadata.assert_called_once_with(filepath=temp_file)  # type: ignore[attr-defined]
...
async def test_extract_metadata_from_stream(mock_transaction: MagicMock, mocker: MockerFixture):
    ...
    with mocker.patch.object(exiftool_instance, "get_metadata", new_callable=mocker.AsyncMock) as mock_get_metadata:
        ...
        assert mock_get_metadata.call_count == 1  # type: ignore[attr-defined]
