...
# The above import is problematic as these are not all exported or are test-specific.
# Tests in this file should ideally use fixtures from conftest.py (e.g., db_session, test_world_alpha).
# For now, removing to allow collection. Runtime errors in this file are expected.
# from .test_transcode_service import db_session # Removed: db_session will be injected from conftest.py


@pytest.mark.asyncio
async def test_create_evaluation_run_concept(test_world_alpha: World, db_session: AsyncSession):
    ...
    async with test_world_alpha.db_session_maker() as session:
        ...
        tags = await tag_service.get_tags_for_entity(test_world_alpha, entity_id=run_entity.id, session=session)
        assert any(tag.name == "System:EvaluationRun" for tag, _ in tags)


@pytest.mark.asyncio
async def test_create_evaluation_run_already_exists(test_world_alpha: World, db_session: AsyncSession):
    run_name = "eval_run_unique_002"
    await evaluation_systems.create_evaluation_run_concept(test_world_alpha, run_name)
...
@pytest.mark.asyncio
async def test_get_evaluation_run_by_name_or_id(test_world_alpha: World, db_session: AsyncSession):
    run_name = "eval_run_to_get"
    created_run_entity = await evaluation_systems.create_evaluation_run_concept(test_world_alpha, run_name)

    # By name
    entity_by_name, comp_by_name = await evaluation_systems.get_evaluation_run_by_name_or_id(test_world_alpha, run_name)
    assert entity_by_name.id == created_run_entity.id
    assert comp_by_name.run_name == run_name

    # By ID
    entity_by_id, comp_by_id = await evaluation_systems.get_evaluation_run_by_name_or_id(test_world_alpha, created_run_entity.id)
    ...
    with pytest.raises(evaluation_systems.EvaluationError, match="Evaluation run 'non_existent_run' not found"):
        await evaluation_systems.get_evaluation_run_by_name_or_id(test_world_alpha, "non_existent_run")
    with pytest.raises(evaluation_systems.EvaluationError, match="Evaluation run '99988' not found"):
        await evaluation_systems.get_evaluation_run_by_name_or_id(test_world_alpha, 99988)


@pytest_asyncio.fixture
async def setup_for_eval_execution(test_world_alpha: World, sample_text_file: Path, db_session: AsyncSession):
    # Create a dummy asset entity for the sample_text_file
    source_asset_entity = Entity()
    db_session.add(source_asset_entity)
    await db_session.flush()
    source_asset_id = source_asset_entity.id

    # Add FilePropertiesComponent for the sample_text_file
    fpc = FilePropertiesComponent(
        entity=source_asset_entity, # Pass entity object
        original_filename=sample_text_file.name,
        file_size_bytes=sample_text_file.stat().st_size,
        mime_type="text/plain"
    )
    db_session.add(fpc)
    # Add FileLocationComponent as it's often needed by services like _get_source_asset_filepath
    flc = FileLocationComponent(
        entity=source_asset_entity, # Pass entity object
        content_identifier = f"sha256_for_{sample_text_file.name}", # Mock hash
        storage_type = "local_cas", # Mock storage type
        physical_path_or_key = str(sample_text_file.relative_to(sample_text_file.parent.parent)), # Mock relative path
        contextual_filename = sample_text_file.name
    )
    db_session.add(flc)
    ...
    profile1_entity = await transcode_service.create_transcode_profile(
        test_world_alpha, profile1_name, "ffmpeg", "-preset ultrafast {output}", "mkv"
    )
    profile2_name = "eval_prof_slow"
    profile2_entity = await transcode_service.create_transcode_profile(
        test_world_alpha, profile2_name, "ffmpeg", "-preset veryslow {output}", "mp4"
    )

    # Create an evaluation run concept
    eval_run_name = "MyVideoEvaluation" # Even if source is text, concept is generic
    eval_run_entity = await evaluation_systems.create_evaluation_run_concept(
        test_world_alpha, eval_run_name, "Evaluating fast vs slow presets"
    )

    return {
        "source_asset_id": source_asset_id,
        "source_asset_path": sample_text_file, # Added path for reference
...
async def test_execute_evaluation_run(
    ...
    test_world_alpha: World, # Changed test_world
    ...
    async def side_effect_apply_transcode(world, source_asset_entity_id, profile_entity_id, output_parent_dir=None):
        ...
        async with world.db_session_maker() as session:
            ...
            fpc = FilePropertiesComponent(
                entity=new_mock_entity, # Pass entity object
                original_filename=f"transcoded_by_mock_{profile_entity_id}.dat",
                file_size_bytes=mock_filesize,
                mime_type="application/octet-stream"
            )
            # fpc.entity_id = new_mock_entity.id # No longer set directly
            session.add(fpc)
            await session.commit() # Commit here to persist FPC before TVC if TVC relies on it
            await session.refresh(new_mock_entity) # Ensure new_mock_entity is refreshed if needed by TVC
    ...
    results_components = await evaluation_systems.execute_evaluation_run(
        world=test_world_alpha, # Changed test_world
    ...
    async with test_world_alpha.db_session_maker() as session: # Changed test_world
...
@pytest.mark.asyncio
async def test_execute_evaluation_run_no_valid_assets_or_profiles(test_world_alpha: World, setup_for_eval_execution: dict): # Changed test_world
    eval_data = setup_for_eval_execution
    ...
    with patch("dam.services.ecs_service.find_entity_id_by_hash", AsyncMock(return_value=None)): # Make hash lookup fail
        with pytest.raises(evaluation_systems.EvaluationError, match="No valid source assets found"):
            await evaluation_systems.execute_evaluation_run(
                test_world_alpha, eval_data["eval_run_id"], ["non_existent_hash"], [eval_data["profile1_id"]]
            )

    # No valid profiles
    with pytest.raises(evaluation_systems.EvaluationError, match="No valid transcode profiles found"):
        await evaluation_systems.execute_evaluation_run(
            test_world_alpha, eval_data["eval_run_id"], [eval_data["source_asset_id"]], ["non_existent_profile_name"]
        )
...
async def test_get_evaluation_results_formatting(
    ...
    test_world_alpha: World, # Changed test_world
    setup_for_eval_execution: dict
    # sample_asset_entity: tuple[int, Path] # This was problematic, get path from setup_for_eval_execution
):
    eval_data = setup_for_eval_execution
    source_asset_orig_path = eval_data["source_asset_path"] # Get original path for filename
    ...
    async def side_effect_apply_transcode(world, source_asset_entity_id, profile_entity_id, output_parent_dir=None):
        async with world.db_session_maker() as session:
            ...
            tvc = TranscodedVariantComponent(
                entity=new_mock_entity, # Pass entity object
                original_asset_entity_id=source_asset_entity_id,
                transcode_profile_entity_id=profile_entity_id
            )
            # tvc.entity_id = new_mock_entity.id # No longer set directly
            session.add(tvc)
    ...
    formatted_results = await evaluation_systems.get_evaluation_results(
        test_world_alpha, eval_data["eval_run_name"] # Changed test_world
    )
    ...
    with pytest.raises(evaluation_systems.EvaluationError, match="Evaluation run 'ghost_run' not found"):
        await evaluation_systems.get_evaluation_results(test_world_alpha, "ghost_run") # Changed test_world


@pytest.mark.asyncio
async def test_execute_evaluation_run_source_asset_not_found(test_world_alpha: World, setup_for_eval_execution: dict): # Changed test_world
    eval_data = setup_for_eval_execution
    ...
    with patch("dam.services.ecs_service.find_entity_id_by_hash", AsyncMock(return_value=None)):
        ...
        results = await evaluation_systems.execute_evaluation_run(
            test_world_alpha, # Changed test_world
        ...
        async def side_effect_atp_valid(world, source_asset_entity_id, profile_entity_id, output_parent_dir=None):
            if source_asset_entity_id == valid_asset_id: # Only mock for the valid one
                async with world.db_session_maker() as session:
                    ...
                    fpc = FilePropertiesComponent(entity=new_mock_entity, original_filename="valid_transcode.dat", file_size_bytes=100) # Pass entity object
