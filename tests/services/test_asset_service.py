...
from dam.models import Entity, ImagePerceptualHashComponent, ContentHashComponent, FileLocationComponent, FilePropertiesComponent
from dam.core.database import SessionLocal, create_db_and_tables, engine # For direct session and engine access if needed
from dam.models import Base # For table setup/teardown
...
def test_add_existing_image_content_adds_missing_perceptual_hashes(db_session: Session, sample_image_a: Path, tmp_path: Path):
    """
    ...
    if not {"phash", "ahash", "dhash"}.issubset(initial_hash_types):
        pytest.skip(f"Initial add did not generate all expected hash types ({initial_hash_types}), cannot test missing hash addition.")
    ...
    entity2, created_new2 = asset_service.add_asset_file(db_session, copy_of_sample_image_a, props2[0], props2[2], props2[1], chash2)
    db_session.commit()
